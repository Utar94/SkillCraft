<template>
  <b-container>
    <h1 v-t="'languages.title'" />
    <div class="my-2">
      <icon-button class="mx-1" :disabled="loading" icon="sync-alt" :loading="loading" text="actions.refresh" variant="primary" @click="refresh()" />
      <icon-button class="mx-1" icon="plus" text="actions.create" :to="{ name: 'LanguageEdit', params: { id: 'new' } }" variant="success" />
    </div>
    <b-row>
      ...
      <table class="table table-striped">
        <thead>
          <tr>
            ...
            <th scope="col" v-t="'language.script.label'" />
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            ...
            <td>
              <icon-button class="mx-1" icon="trash-alt" text="actions.delete" variant="danger" v-b-modal="`deleteLanguage_${item.id}`" />
              <delete-modal
                confirm="languages.delete.confirm"
                :disabled="loading"
                :displayName="item.name"
                :id="`deleteLanguage_${item.id}`"
                :loading="loading"
...
<script>
import { deleteLanguage, getLanguages } from '@/api/languages'
...
export default {
  ...
  computed: {
    ...
    sortOptions() {
      return Object.entries(this.$i18n.t('languages.sort'))
  ...
  methods: {
    async _delete({ id }, callback = null) {
      if (!this.loading) {
        ...
        try {
          ...
          refresh = true
          this.toast('success', 'languages.delete.success')
    ...
    async refresh(params = null) {
      if (!this.loading) {
        this.loading = true
        try {
          const { data } = await getLanguages(params ?? this.params)
