<template>
  <b-container>
    <h1 v-t="'characters.title'" />
    <div class="my-2">
      <icon-button class="mx-1" :disabled="loading" icon="sync-alt" :loading="loading" text="actions.refresh" variant="primary" @click="refresh()" />
      <icon-button class="mx-1" icon="plus" text="actions.create" :to="{ name: 'CharacterEditStep1', params: { id: 'new' } }" variant="success" />
    </div>
    <b-row>
      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            <td>
              <router-link :to="{ name: 'CharacterEdit', params: { id: item.id } }" v-text="item.name" />
            </td>
            <td>{{ $d(new Date(item.updatedAt || item.createdAt), 'medium') }}</td>
            <td>
              <icon-button class="mx-1" icon="trash-alt" text="actions.delete" variant="danger" v-b-modal="`deleteCharacter_${item.id}`" />
              <delete-modal
                confirm="characters.delete.confirm"
                :disabled="loading"
                :displayName="item.name"
                :id="`deleteCharacter_${item.id}`"
                :loading="loading"
...
export default {
  ...
  computed: {
    ...
    sortOptions() {
      return Object.entries(this.$i18n.t('characters.sort'))
  ...
  methods: {
    async _delete({ id }, callback = null) {
      if (!this.loading) {
        ...
        try {
          ...
          refresh = true
          this.toast('success', 'characters.delete.success')
    ...
    async refresh(params = null) {
      if (!this.loading) {
        this.loading = true
        try {
          const { data } = await getCharacters(params ?? this.params)
