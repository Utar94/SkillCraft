<template>
  <b-container>
    <h1 v-t="'conditions.title'" />
    <div class="my-2">
      <icon-button class="mx-1" :disabled="loading" icon="sync-alt" :loading="loading" text="actions.refresh" variant="primary" @click="refresh()" />
      <icon-button class="mx-1" icon="plus" text="actions.create" :to="{ name: 'ConditionEdit', params: { id: 'new' } }" variant="success" />
    </div>
    <b-row>
      ...
      <table class="table table-striped">
        <thead>
          <tr>
            ...
            <th scope="col" v-t="'condition.maxLevel.label'" />
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            <td>
              <router-link :to="{ name: 'ConditionEdit', params: { id: item.id } }" v-text="item.name" />
            </td>
            <td>{{ item.maxLevel || $t('condition.maxLevel.none') }}</td>
            <td>{{ $d(new Date(item.updatedAt || item.createdAt), 'medium') }}</td>
            <td>
              <icon-button class="mx-1" icon="trash-alt" text="actions.delete" variant="danger" v-b-modal="`deleteCondition_${item.id}`" />
              <delete-modal
                confirm="conditions.delete.confirm"
                :disabled="loading"
                :displayName="item.name"
                :id="`deleteCondition_${item.id}`"
                :loading="loading"
                title="conditions.delete.title"
...
<script>
import { deleteCondition, getConditions } from '@/api/conditions'
...
export default {
  ...
  computed: {
    ...
    sortOptions() {
      return Object.entries(this.$i18n.t('conditions.sort'))
  ...
  methods: {
    async _delete({ id }, callback = null) {
      if (!this.loading) {
        ...
        try {
          ...
          refresh = true
          this.toast('success', 'conditions.delete.success')
    ...
    async refresh(params = null) {
      if (!this.loading) {
        this.loading = true
        try {
          const { data } = await getConditions(params ?? this.params)
