<template>
  <b-container>
    ...
    <div class="my-2">
      ...
      <icon-button class="mx-1" icon="plus" text="actions.create" :to="{ name: 'CasteEdit', params: { id: 'new' } }" variant="success" />
    </div>
    <b-row>
      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            <td>
              <router-link :to="{ name: 'CasteEdit', params: { id: item.id } }" v-text="item.name" />
            </td>
            <td>{{ $t(`skill.options.${item.skill}`) }}</td>
            <td>{{ $d(new Date(item.updatedAt || item.createdAt), 'medium') }}</td>
            <td>
              <icon-button class="mx-1" icon="trash-alt" text="actions.delete" variant="danger" v-b-modal="`deleteCaste_${item.id}`" />
              <delete-modal
                confirm="castes.delete.confirm"
                :disabled="loading"
                :displayName="item.name"
                :id="`deleteCaste_${item.id}`"
                :loading="loading"
                title="castes.delete.title"
...
<script>
import { deleteCaste, getCastes } from '@/api/castes'
...
export default {
  ...
  computed: {
    ...
    sortOptions() {
      return Object.entries(this.$i18n.t('castes.sort'))
  ...
  methods: {
    async _delete({ id }, callback = null) {
      if (!this.loading) {
        ...
        try {
          ...
          refresh = true
          this.toast('success', 'castes.delete.success')
    ...
    async refresh(params = null) {
      if (!this.loading) {
        this.loading = true
        try {
          const { data } = await getCastes(params ?? this.params)
