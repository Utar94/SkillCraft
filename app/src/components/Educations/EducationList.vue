<template>
  <b-container>
    <h1 v-t="'educations.title'" />
    <div class="my-2">
      <icon-button class="mx-1" :disabled="loading" icon="sync-alt" :loading="loading" text="actions.refresh" variant="primary" @click="refresh()" />
      <icon-button class="mx-1" icon="plus" text="actions.create" :to="{ name: 'EducationEdit', params: { id: 'new' } }" variant="success" />
    </div>
    <b-row>
      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            <td>
              <router-link :to="{ name: 'EducationEdit', params: { id: item.id } }" v-text="item.name" />
            </td>
            <td>{{ $t(`skill.options.${item.skill}`) }}</td>
            <td>{{ $d(new Date(item.updatedAt || item.createdAt), 'medium') }}</td>
            <td>
              <icon-button class="mx-1" icon="trash-alt" text="actions.delete" variant="danger" v-b-modal="`deleteEducation_${item.id}`" />
              <delete-modal
                confirm="educations.delete.confirm"
                :disabled="loading"
                :displayName="item.name"
                :id="`deleteEducation_${item.id}`"
                :loading="loading"
                title="educations.delete.title"
...
<script>
import { deleteEducation, getEducations } from '@/api/educations'
...
export default {
  ...
  computed: {
    ...
    sortOptions() {
      return Object.entries(this.$i18n.t('educations.sort'))
  ...
  methods: {
    async _delete({ id }, callback = null) {
      if (!this.loading) {
        ...
        try {
          ...
          refresh = true
          this.toast('success', 'educations.delete.success')
    ...
    async refresh(params = null) {
      if (!this.loading) {
        this.loading = true
        try {
          const { data } = await getEducations(params ?? this.params)
