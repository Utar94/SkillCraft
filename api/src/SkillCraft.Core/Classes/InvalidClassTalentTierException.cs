using Logitar.WebApiToolKit.Core.Exceptions;
using SkillCraft.Core.Talents;
using System.Text;

namespace SkillCraft.Core.Classes
{
  internal class InvalidClassTalentTierException : BadRequestException
  {
    public InvalidClassTalentTierException(int tier, Talent talent)
      : base("InvalidClassTalentTier", GetMessage(tier, talent))
    {
      Talent = talent ?? throw new ArgumentNullException(nameof(talent));
      Tier = tier;
    }

    public Talent Talent { get; }
    public int Tier { get; }

    private static string GetMessage(int tier, Talent talent)
    {
      var message = new StringBuilder();

      message.AppendLine("The class cannot require the specified talent.");
      message.AppendLine($"Class tier: {tier}");
      message.AppendLine($"Talent: {talent} (Tier={talent?.Tier}");

      return message.ToString();
    }
  }
}
